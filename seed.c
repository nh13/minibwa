...
// key_sai0 is an SA/BWT coordinate, bounded by the index length (< 2^40); 5 bytes suffice.
KRADIX_SORT_INIT(mb_sai0, mb_sai_t, key_sai0, 5)
...
// key_anchor is a concatenated-reference coordinate, bounded by the index length (< 2^40); 5 bytes suffice.
KRADIX_SORT_INIT(mb_anchor, mb_anchor_t, key_anchor, 5)
...
void mb_seed_intv(void *km, const mb_idx_t *idx, int32_t len, const uint8_t *seq, int32_t min_len, int32_t max_sub_occ, int32_t min_sub_occ, mb_sai_v *v)
{
	...
	if (idx->b2) { // the b2 backend has no scalar SMEM; route through the batch path with n=1
		...
		mb_seed_intv_batch(km, idx, 1, &len1, (uint8_t *const*)&seq1, min_len, max_sub_occ, min_sub_occ, v);
		return;
	}
	v->n = 0;
